...
def get_safe_filename(name):
    ...
    return safe_name.replace(' ', '_') + "_Credentials.pdf"
...
def main():
    ...
    for sub, staff_list in subsidiaries.items():
        pdf = PDF()
        ...
        # Add Subsidiary Title
        pdf.set_font('Arial', 'B', 14)
        pdf.set_text_color(37, 99, 235)
        pdf.cell(0, 10, sub.upper(), 0, 1, 'L')
        pdf.set_font('Arial', '', 10)
        pdf.set_text_color(100, 116, 139)
        pdf.cell(0, 6, f'Total Staff: {len(staff_list)}', 0, 1, 'L')
        pdf.ln(5)
        ...
        filename = get_safe_filename(sub)
